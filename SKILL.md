...
用户请求: "给李四发消息"
                    │
                    ▼
步骤1: get_ocr_result ──→ 检查当前页面是否有"李四"
                    │
                    ▼
        ┌───────────┴───────────┐
        │ 当前列表有"李四"？      │
...
3. **输入联系人名字**
```bash
python {baseDir}/agent.py click_and_type --content "李四"
```
...
```bash
# 假设"李四"在搜索结果中 center 为 [2200, 450]
python {baseDir}/agent.py click_coordinate --x 2200 --y 450
```
...
**注意事项：**
- 搜索框位置：通常在微信窗口左上区域
- 搜索结果可能包含多个匹配项（联系人、群聊、公众号等）
- 如果搜索结果为空，说明联系人不存在或名字错误
- 可以输入部分名字进行模糊搜索
