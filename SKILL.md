---
name: wechat_tool
description: 微信客户端自动化控制工具，支持截图、OCR识别、点击、输入、滚动、发送消息、检查新消息等操作
version: 4.2.0
author: xzg
permissions: 系统操作权限（控制微信窗口、鼠标键盘操作）
---

# 微信自动化技能

## 1. Description（技能详细说明）

这是一个微信客户端自动化控制工具，可以唤醒微信窗口、截图、识别界面内容、模拟点击和输入操作、发送消息、检查新消息。

**注意：微信需要用户手动打开，工具不会自动启动微信。**

**核心特性：**
- 自动唤醒微信窗口（Ctrl+Alt+W 快捷键，仅当微信在托盘区时）
- 所有操作前自动激活窗口，确保窗口在前台
- 每次操作都获取最新截图，不使用缓存
- 支持多窗口标题匹配（"微信"、"WeChat"、"Weixin"）
- **联系人列表自动解析：check_new_messages() 返回已解析的联系人列表（名称、预览、时间、点击坐标）**
- **新消息检测：对比预览消息变化，直接判断是否有新消息**

**⚠️ 跳过的联系人类型：**
- 文件传输助手、File Transfer、文件传输
- 公众号、服务号、微信支付
- 腾讯、系统、官方

## 2. When to use（触发场景）

当用户说以下内容时，使用此技能：

- "打开微信"、"唤醒微信"、"显示微信"
- "帮我截个微信的图"、"微信截图"
- "微信界面上有什么内容"、"识别微信界面文字"
- "点击微信上的XXX"、"在微信里输入XXX"
- "发送消息"、"在当前聊天发送XXX"
- "滚动微信聊天记录"
- "帮我操作微信"相关任务
- "微信状态"、"微信是否在运行"
- **"看看谁给我发消息了"、"查看聊天列表"、"检查新消息"** → 使用 `get_chat_list` 或 `check_new_messages`
- **"查看和XXX的聊天记录"、"看看和XXX的聊天"、"读取XXX的聊天内容"** → 使用完整流程（见第5.1节）
- **"给XXX发消息"、"和XXX说话"、"告诉XXX"** → 使用完整流程（见第5.2节）
- **"回复XXX"** → 使用 `auto_reply`

## 3. How to use（调用逻辑）

**调用格式：**
```bash
python {baseDir}/agent.py <action> [--参数名 参数值]...
```

**重要说明：**
- `<action>` 是动作名称，必须是以下支持的动作之一
- 参数使用 `--参数名 参数值` 格式传递，无需 JSON
- 必需参数必须提供，可选参数可不传
- 所有命令返回 JSON 格式结果

---

### 3.1 screenshot - 截图

**功能：** 截取微信窗口截图

**调用：**
```bash
python {baseDir}/agent.py screenshot
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --save_path | string | 否 | 无 | 保存路径。不传则返回 base64 编码 |

---

### 3.2 get_wechat_status - 获取微信状态

**功能：** 检查微信是否运行及窗口状态

**调用：**
```bash
python {baseDir}/agent.py get_wechat_status
```

**参数说明：** 无参数

---

### 3.3 get_ocr_result - OCR 识别

**功能：** 识别微信界面文字并返回坐标信息

**调用：**
```bash
python {baseDir}/agent.py get_ocr_result
```

**参数说明：** 无参数

---

### 3.4 click_coordinate - 点击坐标

**功能：** 点击屏幕指定坐标位置

**调用：**
```bash
python {baseDir}/agent.py click_coordinate --x 200 --y 150
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --x | int | **是** | - | 屏幕 X 坐标（绝对坐标） |
|| --y | int | **是** | - | 屏幕 Y 坐标（绝对坐标） |

---

### 3.5 click_and_type - 输入文字

**功能：** 在指定位置输入文字（可选先点击再输入）

**调用：**
```bash
python {baseDir}/agent.py click_and_type --content "你好世界"
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --content | string | **是** | - | 要输入的文字内容 |
|| --x | int | 否 | null | 点击位置 X 坐标，不传则不点击 |
|| --y | int | 否 | null | 点击位置 Y 坐标，不传则不点击 |
|| --send_enter | flag | 否 | false | 输入后是否按回车键 |

---

### 3.6 send_message - 发送消息

**功能：** 在当前聊天窗口发送消息（自动定位输入框并发送）

**调用：**
```bash
python {baseDir}/agent.py send_message --message "你好"
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --message | string | **是** | - | 要发送的消息内容 |

---

### 3.7 send_image - 发送图片

**功能：** 在当前聊天窗口发送图片

**调用：**
```bash
python {baseDir}/agent.py send_image --path "C:/Users/Administrator/Desktop/sd/1.png"
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --path | string | **是** | - | 图片文件的绝对路径 |

---

### 3.8 scroll - 滚动页面

**功能：** 滚动微信聊天记录或列表

**调用：**
```bash
python {baseDir}/agent.py scroll --direction down --amount 300
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --direction | string | 否 | "down" | 滚动方向："up" 或 "down" |
|| --amount | int | 否 | 300 | 滚动量（像素） |
|| --x | int | 否 | 窗口中心 X | 滚动位置 X 坐标 |
|| --y | int | 否 | 窗口中心 Y | 滚动位置 Y 坐标 |

---

### 3.9 get_page_context - 获取页面上下文

**功能：** 获取当前微信页面的 OCR 识别结果

**调用：**
```bash
python {baseDir}/agent.py get_page_context
```

**参数说明：** 无参数

---

### 3.10 get_chat_list - 获取聊天列表（OCR原始结果）

**功能：** 获取微信窗口的OCR识别原始结果（用于调试或特殊场景）

**调用：**
```bash
python {baseDir}/agent.py get_chat_list
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --count | int | 否 | 5 | 保留参数（兼容旧接口） |

**返回值：**

成功：
```json
{
  "status": "success",
  "data": {
    "ocr_results": [
      {
        "text": "识别的文本",
        "confidence": 0.95,
        "position": {"x": 100, "y": 200},
        "box": [[x1,y1], [x2,y2], [x3,y3], [x4,y4]]
      }
    ],
    "window_rect": {"left": 0, "top": 0, "width": 800, "height": 600},
    "total": 10
  }
}
```

**说明：**
- 返回原始OCR结果，需要AI自行分析
- **一般情况下推荐使用 `check_new_messages`**，它返回已解析的联系人列表
- 此方法主要用于调试或需要原始OCR数据的特殊场景

---

### 3.11 click_contact - 点击指定坐标

**功能：** 点击指定屏幕坐标进入聊天窗口

**调用：**
```bash
python {baseDir}/agent.py click_contact --x 140 --y 150
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --x | int | **是** | 无 | 点击X坐标（屏幕绝对坐标） |
|| --y | int | **是** | 无 | 点击Y坐标（屏幕绝对坐标） |

**注意：** 
- 坐标需要AI从OCR结果中计算得出
- 点击后会自动验证聊天窗口是否打开

---

### 3.12 auto_reply - 自动回复

**功能：** 点击指定坐标并发送回复消息

**调用：**
```bash
python {baseDir}/agent.py auto_reply --x 140 --y 150 --message "好的，收到了"
```

**参数说明：**

|| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||--------|------|------|--------|------|
|| --x | int | **是** | 无 | 点击X坐标（屏幕绝对坐标） |
|| --y | int | **是** | 无 | 点击Y坐标（屏幕绝对坐标） |
|| --message | string | **是** | 无 | 回复内容 |

---

### 3.13 check_new_messages - 检查新消息

**功能：** 获取当前联系人列表（已自动解析出名称、预览、时间、点击坐标）

**调用：**
```bash
python {baseDir}/agent.py check_new_messages
```

**参数说明：** 无参数

**⚠️ 重要：数据持久化说明**

```
聊天记录保存在本地文件 wechat_chat_records.json 中：
- 程序启动时会自动加载该文件
- 之前保存的数据会一直保留
- 如果不初始化，saved_records 会返回历史数据（可能不是当前会话的）

【首次使用或需要重新开始监控时】必须先初始化：
  → 调用 reset_chat_records() 清空历史数据
  → 然后 check_new_messages() 的 saved_records 才是空的
```

**返回值：**

```json
{
  "status": "success",
  "data": {
    "contacts": [
      {
        "name": "张三",
        "x": 127,
        "y": 88,
        "preview": "你好，在吗？",
        "time": "18:10",
        "screen_x": 2157,
        "screen_y": 163
      }
    ],
    "saved_records": {
      "contacts": {
        "张三": {
          "last_opponent_message": "之前的消息",
          "last_opponent_time": "17:30"
        }
      }
    },
    "is_first_init": false,
    "window_rect": {"left": 2030, "top": 75, "width": 920, "height": 648}
  }
}
```

**返回字段说明：**
||| 字段 | 说明 |
|||------|------|
||| `contacts` | 当前联系人列表（已解析），每个联系人包含 name、preview、time、screen_x、screen_y |
||| `saved_records` | 上次保存的记录，用于对比预览消息是否有变化 |
||| `is_first_init` | 是否是首次初始化（本地无记录文件时为 true） |
||| `window_rect` | 微信窗口的位置和大小 |

**用途：**
- 定时监控时获取当前联系人列表
- 对比 `contacts[].preview` 和 `saved_records.contacts[name].last_opponent_message` 判断是否有新消息
- 使用 `screen_x` 和 `screen_y` 直接点击联系人

---

### 3.14 get_chat_records - 获取所有联系人聊天记录

**功能：** 获取当前保存的所有联系人聊天记录

**调用：**
```bash
python {baseDir}/agent.py get_chat_records
```

**返回值：**
```json
{
  "status": "success",
  "data": {
    "chat_records": {
      "contacts_order": ["张三", "李四", "王五"],
      "contacts": {
        "张三": {
          "last_opponent_message": "你好",
          "last_opponent_time": "10:30"
        }
      }
    },
    "contacts_order": ["张三", "李四", "王五"],
    "total": 3
  }
}
```

---

### 3.15 save_chat_record - 保存单个联系人聊天记录

**功能：** 保存单个联系人的最后一条对方消息

**调用：**
```bash
python {baseDir}/agent.py save_chat_record --name "张三" --message "你好" --time "10:30" --order 0
```

**参数说明：**

||| 参数名 | 类型 | 必需 | 默认值 | 说明 |
|||--------|------|------|--------|------|
||| --name | string | **是** | 无 | 联系人名称 |
||| --message | string | **是** | 无 | 对方发的最后一条消息内容 |
||| --time | string | 否 | "" | 消息时间 |
||| --order | int | 否 | 无 | 联系人顺序（0-4） |

---

### 3.16 get_chat_record - 获取单个联系人聊天记录

**功能：** 获取单个联系人的聊天记录

**调用：**
```bash
python {baseDir}/agent.py get_chat_record --name "张三"
```

**返回值：**
```json
{
  "status": "success",
  "data": {
    "contact_name": "张三",
    "record": {
      "last_opponent_message": "你好",
      "last_opponent_time": "10:30"
    },
    "exists": true
  }
}
```

---

### 3.17 update_contacts_order - 更新联系人顺序

**功能：** 更新联系人顺序列表

**调用：**
```bash
python {baseDir}/agent.py update_contacts_order --order '["张三", "李四", "王五", "赵六", "钱七"]'
```

**参数说明：**

||| 参数名 | 类型 | 必需 | 默认值 | 说明 |
|||--------|------|------|--------|------|
||| --order | string | **是** | 无 | 联系人名称JSON数组 |

---

### 3.18 reset_chat_records - 重置所有聊天记录

**功能：** 重置所有聊天记录（删除本地文件）

**调用：**
```bash
python {baseDir}/agent.py reset_chat_records
```

**注意：** 每次启动定时监控时，必须先调用此命令初始化！

---

### 3.19 update_preview_after_reply - 回复后更新预览消息

**功能：** 回复完成后更新联系人状态，记录回复内容防止下次误判为新消息

**调用：**
```bash
python {baseDir}/agent.py update_preview_after_reply --name "张三" --reply_message "好的，收到了"
```

**参数说明：**

|||| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||||--------|------|------|--------|------|
|||| --name | string | **是** | 无 | 联系人名称 |

**返回值：**
```json
{
  "status": "success",
  "message": "已更新联系人 '张三' 的预览消息",
  "data": {
    "contact_name": "张三",
    "preview": "[我]: 收到了",
    "time": "16:30"
  }
}
```

**重要说明：**
- ⚠️ **必须在回复消息后立即调用此方法**
- ⚠️ **必须传入 --reply_message 参数，记录回复内容**
- 下次检测时会跳过包含此回复内容的预览
- 防止自己的回复消息变成预览后，下次检测时误判为新消息

---

### 3.20 verify_chat_window - 验证聊天窗口

**功能：** 验证当前聊天窗口是否是指定联系人（通过OCR识别对话框顶部名称）

**调用：**
```bash
python {baseDir}/agent.py verify_chat_window --expected_name "张三"
```

**参数说明：**

|||| 参数名 | 类型 | 必需 | 默认值 | 说明 |
||||--------|------|------|--------|------|
|||| --expected_name | string | **是** | 无 | 预期的联系人名称 |

**返回值：**
```json
{
  "status": "success",
  "data": {
    "actual_name": "张三",
    "expected_name": "张三",
    "matched": true
  }
}
```

**用途：**
- 点击联系人进入聊天窗口后，验证是否进入了正确的对话框
- 支持部分匹配（如 "CF-李四" 匹配 "CF-李四@千帆电商"）

---

## 4. 支持的动作列表

|| 动作 | 说明 | 必需参数 | 可选参数 |
||------|------|----------|----------|
|| screenshot | 截图 | 无 | --save_path |
|| get_wechat_status | 获取微信状态 | 无 | 无 |
|| get_ocr_result | OCR 识别 | 无 | 无 |
|| click_coordinate | 点击坐标 | --x, --y | 无 |
|| click_and_type | 输入文字 | --content | --x, --y, --send_enter |
|| send_message | 发送消息 | --message | 无 |
|| send_image | 发送图片 | --path | 无 |
|| scroll | 滚动页面 | 无 | --direction, --amount, --x, --y |
|| get_page_context | 获取页面上下文 | 无 | 无 |
|| **get_chat_list** | 获取聊天列表(OCR原始结果) | 无 | --count |
|| **click_contact** | 点击指定坐标 | --x, --y | 无 |
|| **auto_reply** | 自动回复 | --x, --y, --message | 无 |
|| **check_new_messages** | 检查新消息 | 无 | 无 |
|| **get_chat_records** | 获取所有聊天记录 | 无 | 无 |
|| **save_chat_record** | 保存聊天记录 | --name, --message | --time, --order |
|| **get_chat_record** | 获取单个聊天记录 | --name | 无 |
|| **update_contacts_order** | 更新联系人顺序 | --order | 无 |
|| **reset_chat_records** | 重置所有聊天记录 | 无 | 无 |
|| **update_preview_after_reply** | 回复后更新预览 | --name | 无 |
|| **verify_chat_window** | 验证聊天窗口 | --expected_name | 无 |

---

## 5. 核心流程：定时监控新消息

### 5.1 整体思路

```
核心流程：
1. 调用 check_new_messages() 获取当前联系人列表（已解析出名称、预览、时间）
2. 对比 saved_records 中保存的预览消息
3. 发现新消息 → 点击联系人 → 验证窗口 → 回复 → 更新预览
4. 新联系人（本地无记录）且有预览消息 → 当做新消息处理
```

**关键改进：**
- 不再需要依次点击每个联系人查看聊天记录
- 直接从联系人列表的预览消息判断是否有新消息
- 预览消息可能被截断，需要回复完整聊天内容时才点击进入对话框

### 5.2 初始化流程（第一次启动监控）

```
步骤1: 重置聊天记录
  → 调用 reset_chat_records()

步骤2: 获取当前联系人列表
  → 调用 check_new_messages()
  → 返回 contacts（已解析的联系人列表，包含 name, preview, time, screen_x, screen_y）
  → is_first_init = true

步骤3: 保存所有联系人的预览消息
  对每个联系人：
    a. 调用 save_chat_record --name "联系人名" --message "预览消息" --time "时间"
    b. 不需要点击进入对话框（预览消息已足够）

步骤4: 更新联系人顺序（可选）
  → 调用 update_contacts_order --order '["张三", "李四", ...]'

步骤5: 设置定时任务
```

### 5.3 定时检查流程

```
步骤1: 获取当前联系人列表
  → 调用 check_new_messages()
  → 返回 contacts（当前联系人列表）和 saved_records（上次保存的记录）

步骤2: 对比预览消息找出新消息
  对每个联系人：
    a. 检查联系人名称是否在 saved_records["contacts"] 中
    b. 如果不存在 → 新联系人，当做新消息处理
    c. 如果存在 → 对比 preview 和 saved_records["contacts"][name]["last_opponent_message"]
    d. 如果 preview 不同 → 有新消息

步骤3: 处理新消息
  对每个有新消息的联系人：
    a. 调用 click_contact --x screen_x --y screen_y 进入对话框
    b. 调用 verify_chat_window --expected_name "联系人名" 验证窗口
    c. （可选）获取完整聊天记录判断是否需要回复
    d. 调用 send_message --message "回复内容" 发送回复
    e. ⚠️ 立即调用 update_preview_after_reply --name "联系人名" 更新预览
       （防止自己的回复变成预览导致下次误判）

步骤4: 设置下一次定时任务
```

### 5.4 区分消息来源的关键规则

```
【右侧聊天窗口 - 气泡位置判断】

1. 获取窗口宽度 window_width
2. 对每条消息计算X坐标中心点：
   center_x = (box[0][0] + box[2][0]) / 2

3. 判断消息来源：
   - center_x > window_width * 0.6 → 我发的消息（右侧气泡）
   - center_x < window_width * 0.5 → 对方发的消息（左侧气泡）
   - 中间区域需要根据上下文判断

4. 找对方最后一条消息：
   - 过滤出 sender="对方" 的消息
   - 按Y坐标排序，取最后一条（Y最大的）

【关键规则】
- 只有"对方发的消息"才需要回复
- 如果判断是"我发的消息"，绝对不要回复！
- 不确定时，默认假设是"我发的消息"，不回复
```

---

## 6. 常见操作流程

### 6.1 查看聊天记录流程

**场景：用户说"查看和XXX的聊天记录"、"看看和XXX的聊天"**

**标准流程：**
```
步骤1: 获取OCR结果
  → 调用 get_chat_list() 获取原始OCR结果

步骤2: AI分析OCR结果
  → 根据X坐标区分左侧列表和右侧聊天窗口
  → 在左侧列表中找到目标联系人名称
  → 计算联系人点击坐标

步骤3: 点击联系人进入聊天窗口
  → 调用 click_contact --x 坐标 --y 坐标
  → ⚠️ 注意：点击后需要验证是否成功进入聊天窗口

步骤4: 验证聊天窗口状态（重要！）
  → 点击联系人后，再次调用 get_chat_list() 获取OCR结果
  → 检查右侧是否出现聊天窗口
  → 如果右侧没有聊天窗口，说明可能已经在该聊天窗口，点击反而关闭了
  → 如果确认聊天窗口被关闭，再次点击联系人重新打开

步骤5: 查看聊天记录
  → AI分析OCR结果，根据Y坐标区分不同消息
  → 区分我发的消息（右侧）和对方发的消息（左侧）
  → 或通过 scroll() 滚动查看更多历史记录
```

---

### 6.2 发送消息流程

**场景：用户说"给XXX发消息"、"和XXX说话"、"告诉XXX"**

**标准流程：**
```
步骤1: 获取当前界面状态
  → 调用 get_chat_list() 获取OCR结果
  
步骤2: AI分析当前状态
  → 判断当前是否已在目标聊天窗口
  → 检查右侧聊天窗口顶部是否显示目标联系人名称
  
步骤3: 根据判断结果执行不同操作

  情况A - 已经在目标聊天窗口：
    → 直接跳到步骤5发送消息
    → ⚠️ 不要点击联系人，否则会关闭窗口！

  情况B - 不在目标聊天窗口：
    → 执行步骤4，点击联系人进入聊天窗口

步骤4: 点击联系人并验证窗口状态（关键！）
  → AI计算联系人坐标
  → 调用 click_contact --x 坐标 --y 坐标
  → 再次获取OCR结果验证窗口状态

步骤5: 确认窗口打开后发送消息
  → 调用 send_message --message "内容"
```

---

### 6.3 检查新消息并回复流程

**场景：用户说"看看有没有新消息"、"检查消息"**

**标准流程：**
```
步骤1: 获取OCR结果
  → 调用 check_new_messages() 获取OCR结果

步骤2: AI分析OCR结果
  → 区分左侧列表和右侧聊天窗口
  → 识别联系人名称和消息内容
  → 判断消息来源（我发的/对方发的）

步骤3: 用户选择回复
  → AI计算联系人坐标
  → 调用 auto_reply --x 坐标 --y 坐标 --message "回复内容"
```

---

## 7. 重要机制说明

### 7.1 坐标系统说明

**关键规则：**
- 所有坐标均为屏幕绝对坐标，左上角为 (0, 0)
- `check_new_messages()` 已自动计算 `screen_x` 和 `screen_y`，可直接用于点击
- 右侧聊天窗口气泡位置判断：X > 60% 是我发的，X < 50% 是对方发的

---

### 7.2 定时监控消息的正确使用方式

**⚠️ 极其重要：定时任务触发时必须先初始化状态！**

**使用 cron.add 工具设置定时任务：**

```
【第一次启动监控时】必须初始化！

步骤1: 重置状态
  → 调用 reset_chat_records()

步骤2: 获取当前联系人列表
  → 调用 check_new_messages()
  → 返回 contacts（已解析的联系人列表）

步骤3: 保存所有联系人的预览消息
  对每个联系人：
    → 调用 save_chat_record --name "联系人名" --message "预览消息" --time "时间"

步骤4: 设置定时任务
  → 使用 cron.add 工具设置下次检查时间
  → schedule.at = 当前时间 + 间隔时间（ISO 8601 格式）
  → payload.kind = "agentTurn"
  → payload.message = "检查微信新消息"

【定时任务触发时】对比消息变化！

步骤1: 获取当前联系人列表
  → 调用 check_new_messages()
  → 返回 contacts 和 saved_records

步骤2: ⚠️ 过滤不需要处理的联系人

  跳过以下类型的联系人（不需要回复）：
  - 文件传输助手、File Transfer、文件传输
  - 公众号、服务号、微信支付
  - 腾讯、系统、官方
  - 折叠聊天、折叠的聊天
  - 包含以上关键词的联系人名称


步骤3: 对比预览消息（重要：防止误判自己发送的消息）

  ⚠️ 预览消息会被截断！微信列表预览只显示约20-30字，需使用"包含匹配"

  对每个联系人：
    a. 检查联系人名称是否在 saved_records["contacts"] 中
    b. 如果不存在 → 新联系人，当做新消息处理
    c. 如果存在 → 检查 last_reply_message 字段
       - 如果 last_opponent_message 为空，说明上次已处理，不是新消息
       - 如果 preview 被 last_reply_message 包含（预览是回复的开头部分），说明是自己发的，不是新消息
       - 例如：reply_message="今天天气真好，阳光明媚"，preview="今天天气真好..." → 匹配，不是新消息
    d. 只有当 last_opponent_message 非空，且 preview 与之不同时 → 有新消息

步骤4: 批量处理新消息
  ⚠️ 重要：必须一次性处理所有新消息，中间不要再次获取OCR！

  → 在 AI 内存中记录所有【有新消息】的联系人名称和坐标
  → 按从上到下的顺序逐一处理：
    - auto_reply --x screen_x --y screen_y --message "回复内容"
    - update_preview_after_reply --name "联系人名" --reply_message "回复内容"

步骤5: 设置下一次定时任务
```

**⚠️ 关于新联系人的处理：**

```
【新联系人判断】
如果联系人名称不在 saved_records["contacts"] 中：
- 这是新出现在列表中的联系人
- 通常是因为对方发来了新消息
- 应该当做新消息处理

【示例】
假设 saved_records 中有：张三、李四、王五
当前列表出现：张三、李四、赵六（赵六是新联系人）
→ 赵六不在保存记录中 → 当做新消息处理
```

**⚠️ 为什么不能中间获取 OCR？**

```
错误做法：
1. OCR 获取状态，发现 A、B、C 有新消息
2. 点击 A 回复 → 右侧窗口变为 A 的聊天
3. 再次 OCR 获取状态 → ❌ 错误！
   - 右侧窗口已经不是初始状态
   - 左侧列表可能已经滚动变化
   - 之前计算的 B、C 坐标可能失效

正确做法：
1. OCR 获取状态，发现 A、B、C 有新消息
2. 在 AI 内存中记录：A(坐标1), B(坐标2), C(坐标3)
3. 直接用记录的坐标：
   - auto_reply --x 坐标1 --y 坐标1 --message "回复A"
   - update_preview_after_reply --name "A" --reply_message "回复A"  ← 回复后立即更新
   - auto_reply --x 坐标2 --y 坐标2 --message "回复B"
   - update_preview_after_reply --name "B" --reply_message "回复B"  ← 回复后立即更新
   - auto_reply --x 坐标3 --y 坐标3 --message "回复C"
   - update_preview_after_reply --name "C" --reply_message "回复C"  ← 回复后立即更新
```

**⚠️ 点击坐标已自动计算**

```
check_new_messages() 返回的每个联系人已包含：
- screen_x: 屏幕绝对X坐标（可直接用于点击）
- screen_y: 屏幕绝对Y坐标（可直接用于点击）

使用方法：
click_contact --x contact["screen_x"] --y contact["screen_y"]
auto_reply --x contact["screen_x"] --y contact["screen_y"] --message "回复内容"
```

---

### 7.3 自动回复规则

**模型检测到新消息后，必须自动回复，遵循以下规则：**

```
【重要原则】

⚠️ 没有默认回复话术！每条消息都要根据具体内容思考后回复。

【回复内容生成规则】

1. 理解消息内容 → 思考后针对性回复
   - 根据对方说的具体内容进行分析
   - 结合上下文理解对方意图
   - 回复要有针对性，不要套用模板

2. 能办到的事情 → 执行并回复结果
   - 查询信息 → 调用工具查询后回复具体结果
   - 简单计算 → 计算后回复答案
   - 设置提醒 → 设置后确认

3. 办不到的事情 → 明确告知原因
   - 说明具体原因，不要敷衍

4. 不理解的消息 → 回复固定话术
   - 统一回复："不明白您的意思"
   - 等待对方进一步说明
```

---

## 8. 注意事项

1. **坐标系统**：所有坐标均为屏幕绝对坐标，左上角为 (0, 0)
2. **OCR解析**：由AI根据相对位置智能分析，不再固定解析
3. **窗口大小**：适应不同窗口大小，通过相对比例判断位置
4. **消息发送**：`send_message` 会自动定位输入框，无需手动点击
5. **窗口前置**：所有操作都会自动将微信窗口置顶
6. **无缓存**：每次 OCR 和截图都获取最新画面
7. **消息来源判断**：通过右侧聊天窗口的气泡位置判断，X > 60% 是我发的，X < 50% 是对方发的
8. **聊天记录持久化**：聊天记录保存在 `wechat_chat_records.json` 文件中
9. **定时任务初始化**：设置定时任务时必须先调用 `reset_chat_records` 初始化
10. **批量处理**：check_new_messages 返回所有联系人和坐标，批量处理新消息，中间不要再获取 OCR
11. **中文编码**：脚本输出使用 GBK 编码，适配 Windows CMD 环境
12. **跳过联系人**：文件传输助手、公众号、微信支付等不需要回复
13. **回复后更新预览**：⚠️ 回复消息后必须调用 `update_preview_after_reply` 更新预览，防止自己的回复变成预览导致下次误判为新消息
14. **预览消息截断**：微信列表预览只显示约20-30字，检测时使用"包含匹配"而非"完全匹配"，只要预览是回复内容的开头部分，就认为是自己发的
