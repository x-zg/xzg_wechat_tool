---
name: wechat_tool
description: 微信客户端自动化控制工具，支持截图、OCR识别、点击、输入、滚动、发送消息等操作
version: 1.4.0
author: xzg
...
这是一个微信客户端自动化控制工具，可以唤醒微信窗口、截图、识别界面内容、模拟点击和输入操作、发送消息。
...
**⚠️ 控制命令优先级最高：**
- **"停止"、"停止监控"、"够了"** → **立即执行 `stop_monitor`**，响应速度要快！
- 模型在收到控制命令时必须**立即执行**，不要犹豫，不要询问用户
- 控制命令的优先级高于任何其他正在执行的任务
...
- **"回复XXX"** → 使用 `auto_reply`
- **"停止监控"、"停止"、"不要监控了"、"够了"** → 使用 `stop_monitor`（立即执行！）
...
**注意：** 
- 此命令会自动点击输入框（窗口底部中央位置）
- 使用剪贴板粘贴方式发送图片，然后按回车发送
- 需先打开目标聊天窗口
- 支持 BMP、PNG、JPG、JPEG、GIF 等常见图片格式
...
{
  "status": "success",
  "message": "图片发送成功: C:/Users/Administrator/Desktop/sd/1.png"
}
```
...
{
  "status": "error",
  "message": "图片文件不存在: C:/Users/Administrator/Desktop/sd/1.png"
}
```

**使用场景：**
- 用户说："发送图片 C:/xxx/1.png 给小许" → 先 `click_contact --name "小许"`，再 `send_image --path "C:/xxx/1.png"`
- 用户说："把桌面上的图片发给当前聊天" → 直接 `send_image --path "C:/Users/xxx/Desktop/xxx.png"`
...
### 3.14 check_new_messages - 检查新消息

**功能：** 检查是否有新消息（不自动回复，供模型自定义回复使用）
...
| 消息类型 | 示例 | 处理方式 |
|----------|------|----------|
| **控制型** | "停止监控"、"停止"、"够了"、"不要监控了" | **立即执行 `stop_monitor`**，不要犹豫，不要问用户 |
...
模型判断: 这是控制型消息
    │
    ▼
模型立即执行: stop_monitor（不要犹豫，不要问用户）
    │
    ▼
完成 ✓ 回复用户 "已停止监控"
```
...
**⚠️ 重要：停止监控必须立即执行！**
- 用户说"停止"、"够了"时，模型必须**立即调用 `stop_monitor`**
- 不要询问用户"你确定要停止吗？"
- 不要等待监控自动结束
- 模型反应必须快，否则用户体验很差
...
### 3.16 stop_monitor - 停止聊天监控

**功能：** 停止正在运行的聊天监控

**⚠️ 重要说明：**
- 此命令会终止正在运行的 `start_monitor` 进程
- 如果没有监控在运行，会返回提示信息
- 建议在启动监控时就设置合理的 `--timeout` 和 `--max_loops` 参数

**调用：**
```bash
python {baseDir}/agent.py stop_monitor
```
...
**触发场景：**
- 用户说："停止监控"、"不要再监控了"、"关闭监控"
- 用户说："够了"、"不用继续监控了"

**注意事项：**
- 监控脚本由于是阻塞式运行，模型可能无法立即执行 `stop_monitor` 命令
- 如果模型无法中断，用户可以在终端按 `Ctrl+C` 手动停止
- 建议使用较短的 `--timeout`（如30秒）来避免长时间无法停止
...
3. 如果需要持续监控并自动回复（默认10秒检查一次，60秒超时）
```bash
python {baseDir}/agent.py start_monitor --auto_reply "好的，稍后回复"
```
...
| 场景 | 处理方式 | 用户提示 |
|------|----------|----------|
| 微信未启动 | 返回 `status: "not_running"` | "微信未运行，请手动打开微信" |
| 窗口被最小化 | 自动恢复窗口并置顶 | 无需提示 |
| 窗口被遮挡 | 使用 win32gui 强制置顶 | 无需提示 |
| 找不到窗口 | 尝试 Ctrl+Alt+W 唤醒（托盘区） | "微信未运行，请手动打开微信" |
